...
class Autoreaction(commands.Cog, name='autoreaction'):
    ...
    def __init__(self, client):
        self.client = client
        self.con = sqlite3.connect('databases/autoreactor.db', timeout=5.0)
    ...
    async def autoreact_add(self, ctx, trigger: str = None, response: Union[discord.Emoji, str] = None):
        """
        ...
        if (len(self.cur.execute("SELECT response FROM autoreactions WHERE trigger=? AND guild_id=?", (trigger, ctx.guild.id,)).fetchall()) != 0):
        ...
        self.cur.execute("INSERT INTO autoreactions(guild_id, trigger, response) VALUES (?, ?, ?)", (ctx.guild.id, trigger, reaction,))
        self.con.commit()
    ...
    async def autoreact_remove(self, ctx, trigger: str = None):
        ...
        if trigger is None:
            return await ctx.send("Trigger is a required argument.")
        ar = self.cur.execute("SELECT response FROM autoreactions WHERE trigger=? AND guild_id=?", (trigger, ctx.guild.id,)).fetchall()
        if len(ar) == 0:
            return await ctx.send("Looks like I don't have any reactions for that trigger.")
        self.cur.execute("DELETE FROM autoreactions WHERE trigger=? AND guild_id=?", (trigger, ctx.guild.id,))
        self.con.commit()
    ...
    async def autoreact_clear(self, ctx):
        ...
        """
        ars = self.cur.execute('SELECT trigger FROM autoreactions WHERE guild_id=?', (ctx.guild.id,)).fetchall()
        if len(ars) == 0:
        ...
        if ctx.channel.permissions_for(ctx.me).add_reactions:
            ...
            try:
                ...
                if str(reaction.emoji) == '✅':
                    self.cur.execute("DELETE FROM autoreactions WHERE guild_id=?", (ctx.guild.id,))
                    self.con.commit()
    ...
    async def autoreact_list(self, ctx):
        ...
        Required role: <@&608495204399448066>
        """
        ars = self.cur.execute("SELECT trigger, response FROM autoreactions WHERE guild_id=?", (ctx.guild.id,)).fetchall()
        if len(ars) == 0:
            return await ctx.send("This server does not have any autoreaction.")
    ...
    async def on_message(self, message):
        ...
        if message.author.bot:
            return
        ars = self.cur.execute("SELECT trigger, response FROM autoreactions WHERE guild_id=?", (message.guild.id,)).fetchall()
        if len(ars) == 0:
            return
        ...
        for ar in ars:
            ...
            if trigger in msgcontent:
                try:
                    return await message.add_reaction(ar[1])
                except discord.HTTPException:
                    self.cur.execute("DELETE FROM autoreactions WHERE guild_id=? AND trigger=?", (message.guild.id, trigger))
                    self.con.commit()