...
class polls(commands.Cog):
    ...
    async def on_ready(self):
        if not self.views_added:
            ...
            all_polls = await self.client.db.fetch("SELECT * FROM polls")
    ...
    async def create_poll(self, ctx, anonymous: Optional[Literal['anonymous']] = None, *, question_and_choices = None):
        """
        ...
        if len(another_question_and_choices) != len(question_and_choices):
            return await ctx.send("You cannot have any question and options that have the same name.")
        if len(another_question_and_choices) == 1:
            return await ctx.send(f"You only provided the question, `{another_question_and_choices[0]}`. You need to provide at least 2 options. See `dv.help poll` for more information.")
        if len(another_question_and_choices) < 3:
        ...
        if len(another_question_and_choices) > 6:
        ...
        if anonymous is not None:
            ...
            anonymous = False
        embed = self.generate_embed(f"{ctx.author}'s Poll", ctx.author.display_avatar.url, question, choices_dict, anonymous)
        ...
        await self.client.db.execute("INSERT INTO polls(guild_id, channel_id, invoked_message_id, message_id, creator_id, poll_name, choices, created, anonymous) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", ctx.guild.id, ctx.channel.id, ctx.message.id, poll_view.response.id, ctx.author.id, question, "|".join(choices), timeadded, anonymous)
