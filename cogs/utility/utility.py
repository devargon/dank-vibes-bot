    ...
    async def checkoverwrites(self, ctx: DVVTcontext, channel:discord.TextChannel = None):
        ...
        if channel_details is None:
            ...
            if owner is not None:
                owner_member_type = ctx.guild.get_member(owner.id)
        ...
        if owner is not None:
            ...
            if owner not in channel.overwrites:
                ...
                await ctx.send(f"{owner_member_type}, {type(owner_member_type)}, {owner}, {owner.id}")
        ...
        con = "From 19 December, `dv.pvc` will be a mod-only command.\nTo check your private channel's details, use </privchannel view:1020235323785105418>." if channel.permissions_for(ctx.author).ban_members is False else ""
