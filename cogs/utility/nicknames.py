...
class NicknamePersistentView(discord.ui.View):
    ...
    async def green(self, button: discord.ui.Button, interaction: discord.Interaction):
        ...
        if config is None or config.get('nicknamechannel_id') != interaction.channel_id:
            return
        nickname_request = await self.client.db.fetchrow("SELECT * FROM nicknames WHERE messageid = $1", interaction.message.id)
        ...
        await self.client.db.execute("DELETE from nicknames WHERE id = $1", ID)
...
class nicknames(commands.Cog):
    ...
    async def on_ready(self):
        existing_requests = await self.client.db.fetch("SELECT messageid FROM nicknames")
    ...
    async def setnick(self, ctx, *, nickname = None):
        ...
        if not (config := self.nickconfig.get(ctx.guild.id)):
            config = await self.client.db.fetchrow("SELECT nicknamechannel_id FROM channelconfigs where guild_id = $1", ctx.guild.id)
        ...
        if request_channel is None:
            await self.client.db.execute("DELETE FROM channelconfigs WHERE guild_id = $1", ctx.guild.id)
            return await ctx.send("I could not find the channel to send nickname requests to. Please contact an admin about this!")
        pastnickname = await self.client.db.fetchrow("SELECT * FROM nicknames where member_id = $1", ctx.author.id)
        approveview = NicknamePersistentView(self.client)
        if pastnickname is not None:
            ID = pastnickname.get('id')
            await self.client.db.execute("UPDATE nicknames set nickname = $1 where id = $2", nickname, ID)
            ...
            try:
                ...
                await self.client.db.execute("UPDATE nicknames set messageid = $1 where id = $2", request_message.id, ID)
            else:
                await request_message.edit(embed=requestembed)

        else:
            await self.client.db.execute("INSERT INTO nicknames(member_id, nickname) values($1, $2)", ctx.author.id, nickname)
            ID = (await self.client.db.fetchrow("SELECT id FROM nicknames where member_id = $1 and nickname = $2", ctx.author.id, nickname)).get('id')
            ...
            await self.client.db.execute("UPDATE nicknames set messageid = $1 where id = $2", new_message.id, ID)
