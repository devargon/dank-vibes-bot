...
class timedrole(commands.Cog):
    ...
    async def timedrole(self):
        try:
            ...
            result = await self.client.db.fetch("SELECT * FROM autorole WHERE time < $1", timenow)
            if len(result) == 0:
                return
            for row in result:
                    ...
                    if member is not None:
                        ...
                        if role is not None:
                            try:
                                ...
                                pass
                await self.client.db.execute("DELETE FROM autorole WHERE member_id = $1 and guild_id = $2 and role_id = $3 and time = $4", row.get('member_id'), row.get('guild_id'), row.get('role_id'), row.get('time'))
        except Exception as e:
            print(f"timedrole task caught an error: {e}")