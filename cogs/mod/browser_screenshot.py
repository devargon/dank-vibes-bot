...
class BrowserScreenshot(commands.Cog):
    ...
    async def screenshot(self, ctx, link):
        ...
        def generate_screenshot():
            try:
                ...
                if "This version of ChromeDriver only supports" in str(e):
                    return None, "The version of Chrome used on this device is not matched with ChromeDriver. Please notify the developer about this."
            else:
                #ctx.send(f"<a:typing:839487089304141875> **Getting screenshot** of <{link}>...")
                try:
                    browser.get(link)
                except Exception as e:
                    if "ERR_CONNECTION_TIMED_OUT" in str(e):
                        return None, f"**This site can't be reached**;\n`{domain}` took too long to respond.\n`ERR_CONNECTION_TIMED_OUT`"
                    elif "ERR_CERT_DATE_INVALID" in str(e):
                        return None, f"**Please inform the developer about this.**\n`ERR_CERT_DATE_INVALID`"
                ...
                browser.quit()
                #ctx.send(f"<:greentick:806531672140283944> Saved screenshot with Google Chrome® Version {str1} by Google.")
                #ctx.send(f"<a:typing:839487089304141875> Getting SSL certificate information for the domain {domain}...")
        ...
        async with ctx.typing():
            file, result = await loop.run_in_executor(None, generate_screenshot)
        if file is None:
            if "CERT" in result:
                class View(discord.ui.View):
                    def __init__(self, ctx):
                        super().__init__(timeout=43200)  # the button will stay active for 12 hours, after that it will show interaction failed
        ...
        embed = discord.Embed(title=f"Website details for {domain}", description=result+"\n\nScreenshot: ",
