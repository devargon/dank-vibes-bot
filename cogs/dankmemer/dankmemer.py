    ...
    async def dankmemerreminders(self):
        ...
        for result in results:
            ...
            if config is None: # no config means user doesn't even use this reminder system lol
                pass
            elif result.get('remindertype') not in [2,3,4,5]: # since 2 3 4 5 corresponds to the respective reminders, if somehow an invalid number is inserted it skips straight to deleting
                pass
    ...
    async def on_message(self, message):
        ...
        if "You tryna buy a lottery ticket for ⏣ 5000?" in message.content and message.author.bot and message.author.id == 270904126974590976:
            if len(message.mentions) == 0:
                ...
                def check_lottery(payload):
                    return payload.author == message.mentions[0] and message.channel == payload.channel
                try:
        ...
        if "You equipped a lifesaver. For the next 10" in message.content and message.author.bot and message.author.id == 270904126974590976:
            if len(message.mentions) == 0:
                ...
                member = message.mentions[0]
                nextdailytime = round(time.time()) + 36000
                existing = await self.client.pool_pg.fetch(
                    "SELECT * FROM dankreminders where member_id = $1 and remindertype = $2", member.id, 5)
                if len(existing) > 0:
                    await self.client.pool_pg.execute(
                        "UPDATE dankreminders set time = $1 WHERE member_id = $2 and remindertype = $3", nextdailytime,
                        member.id, 5)
