...
class RemovingAccess(commands.Cog):
    ...
    async def remind_perk_removal(self):
        ...
        for perk in expired_perks:
            ...
            await self.client.get_channel(modchannel).send(f"<:status_online:840918419246415873> {member}'s **{perk.get('perk')}** is over.")
    ...
    async def command_removal(self):
        ...
        try:
            ...
            for commandperk in expired_commands:
                ...
                await self.client.get_channel(modchannel).send(f"<:status_online:840918419246415873> {member}'s **`{commandperk.get('command')}`** is over.")
